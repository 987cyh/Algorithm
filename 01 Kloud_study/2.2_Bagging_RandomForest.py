...
model_baggingtree = BaggingClassifier(DecisionTreeClassifier(max_depth=3), n_estimators=100) # n_estimators : 생성할 의사결정 나무 개수
...
"""변수중요도"""
feature_imp = np.mean([tree.feature_importances_ for tree in model_baggingtree.estimators_], axis=0)
...
"""변수중요도"""
feature_imp = np.mean([tree.feature_importances_ for tree in model_RF.estimators_], axis=0)
n_feature = len(tmp.columns.drop(['종속', '시리얼번호', '온도차이', '온도비율차이']))
idx = np.arange(n_feature)
sorted_idx = feature_imp.argsort()
...
result = permutation_importance(model_RF, x_test, y_test, n_repeats=10,random_state=42, n_jobs=2)
