...
os.chdir('02 bigdata datamining/')
#%%
...
# ★★★ sdot_loc.columns 정제: 칼럼 내 Unnamed 제거 ★★★
cols = list(map(lambda x: tuple(map(lambda y: '' if 'Unnamed' in y else y, x)), sdot_loc.columns))
sdot_loc.columns = list(map(lambda x: ''.join(x), cols)) # join을 통한 col 내 ''제거
...
# for구문으로 데이터를 list에 담아서, 한번에 pd.concat으로 붙이던 방식과 다른 방식

# del sdot_list, cols
#%%
"""위치정보를 geopandas point로 변경"""
gsdot = gpd.GeoDataFrame(sdot_loc[['시리얼번호', '주소', '높이']], geometry=gpd.points_from_xy(sdot_loc['경도'], sdot_loc['위도']))

gsdot = gsdot.set_crs(epsg=4326) # WGS84
gsdot.to_crs(epsg=5174, inplace=True)
...
# 누락값 처리
sdot_pivot.isnull().sum()
sdot_pivot = sdot_pivot.fillna(-40.0)
sdot_pivot.columns = [pd.to_datetime(x, format='%Y%m%d%H%M', errors='ignore') for x in sdot_pivot.columns] # 시간형식으로 변경

"""
axis=0은 각 열(columns)의 모든 행(row)에 대해서 동작
axis=1은 각 행(row)의 모든 열(columns)에 대해서 동작
"""
# 온도 정보가 35회 이상 누락된 "센서" 제거(센서기준: axis=1)
tmp = sdot_pivot[sdot_pivot == -40].count(axis=1) # -40을 기준으로 이상치 제거
tmpcnt = tmp.value_counts().reset_index() # 누락 빈도 확인
tmp = tmp.reset_index()

sdot_pivot = sdot_pivot[sdot_pivot.index.isin(tmp[tmp[0] < 35].시리얼.values)]  # isin과 index를 통한 35회 이상 누락된 센서 제거

# 온도 정보가 1개 이상 누락된 "시간"의 센서 정보는 삭제(시간기준)
# tmp = sdot_pivot[sdot_pivot == -40].count(axis=0) # axis=0
tmp = sdot_pivot[sdot_pivot == -40].count() # axis=0
tmp = tmp.reset_index()
...
gsdot_temp = gsdot.merge(sdot_pivot.reset_index(), how='inner', left_on='시리얼번호', right_on='시리얼') # 교집합 inner

sdot_pivot.describe()
vmax = 40  # max(sdot_pivot.max())
vmin = 10  # min(sdot_pivot.min())

for t in gsdot_temp.columns[6:]: # 시간형식 칼럼만 시각화
...
연속지적['지목'] = 연속지적['지목'].apply(lambda x: re.sub('[0-9\- ]', '', x)) # 지목내 숫자제거
tmp = 연속지적.head(10)
...
""" 각 필지별 참여 비율 계산"""
연속지적['면적'] = 연속지적.geometry.area              # 지적전체: 구적
gsdot_지적['면적'] = gsdot_지적.geometry.area        # 지적편입: 구적

필지면적 = 연속지적[['고유번호', '면적']]
필지면적 = 필지면적.set_index('고유번호')

gsdot_지적['원래면적'] = gsdot_지적['고유번호'].apply(lambda x: 필지면적.loc[x])  # 연속지적도를 첨에 다 계산하면 시간 및 메모리의 비효율성 ★★★
...
표제부['고유번호'] = 표제부.apply(lambda x : x['시군구코드'] + x['법정동코드'] + str(int(x['대지구분'])+1) + x['본번'] + x['부번'], axis=1) # PNU 만들기
