...
data.info()
data.columns
cols_error = ['Mileage', 'Engine', 'Power'] # value 내 단위 정제
...
data.info()
data = data.groupby('Company').apply(lambda x: x.fillna(x.mean())) # 기업을 기준으로 평균으로 대치
data.isnull().sum() # 누락값 확인

data['Power'] = data['Power'].fillna(data['Power'].mean()) # 단일 기업이 누락
data.info()
...
sns.heatmap(data=data[['Log_Kilometers_Driven','Log_Engine','Log_Power','Log_Price','Mileage']].corr(), annot=True)
plt.show()
